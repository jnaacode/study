...
public class MainView { //메인메뉴 (메뉴선택용 / 입력 화면)
	
	private Scanner sc = new Scanner(System.in);
	//로그인한 회원의 정보가 저장된 객체를 참조할 변수 
	private Member loginMember = null; // heap메모리에 loginMember에 null값이 있음! 안써도 됨  
	...
	public void displayMenu() {
		...
		int memuNum = -1; //메뉴 선택 값 저장 변수 
		// 초기값을 -1로 지정하여, 반복문 첫 바퀴에서 오류 발생시 종료되는 것을 방지 
		...
		do {
			...
			try {
				...
				if(loginMember == null) { //로그인이 되어 있지 않은 경우 
					...
					memuNum = sc.nextInt(); // InputMismatchException
					...
					case 1: loginMember=memberView.login();break; //멤버뷰에 있는 로그인메소드를 호출해라 
					case 2: memberView.signUp();break; // 멤버뷰에 있는 사인업를 호출해라
