...
-- [관리자 계정] member 계정 생성
CREATE USER pja_member IDENTIFIED BY member1234;

-- [관리자 계정] CONNECT, RESOURCE + CREATE VIEW 권한 부여 + 객체 생성 공간 할당
GRANT CONNECT,RESOURCE, CREATE VIEW TO pja_member;
ALTER USER pja_member DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;

-- pja_member 계정 접속 방법 추가 
...
COMMENT ON COLUMN MEMBER.MEMER_ID IS '회원 아이디';
COMMENT ON COLUMN MEMBER.MEMBER_PW IS '회원 비밀번호';
COMMENT ON COLUMN MEMBER.MEMBER_NM IS '회원 이름';
COMMENT ON COLUMN MEMBER.MEMBER_GENDER IS '회원 성별(M/F)';
COMMENT ON COLUMN MEMBER.ENROLL_DATE IS '회원 가입일';
COMMENT ON COLUMN MEMBER.SECESSION_FL IS '탈퇴여부일(Y/N)';

-- 게시판 테이블 
DROP TABLE BOARD; -- 잘못 만들었을 경우 
...
COMMENT ON COLUMN BOARD.BOARD_TITLE IS '게시글제복';
COMMENT ON COLUMN BOARD.BOARD_CONTENT IS '게시글내용';
COMMENT ON COLUMN BOARD.CREATE_DATE IS '게시글작성일';
COMMENT ON COLUMN BOARD.READ_COUNT IS '게시글조회수';
...
COMMENT ON COLUMN REPLY.REPLY_NO IS '댓글번호(PK)';
...
UPDATE MEMBER 
SET SECESSION_FL = 'Y'
WHERE MEMBER_NO = '1' -- 전제 로그아웃되지 않게 조건을 달아줘야함
AND MEMBER_PW = 'pass1';


-------------게시글 목록 조회  +댓글 개수(상관 서브쿼리 + 스칼라 서브쿼리)
-- BOARD 테이블 : BOARD_NO,BORAD_TITLE,BOARD_CONTENT,CREATE_DATE,READ_COUNT,MEMBER_NO
...
-- 댓글 목록에서 최근 작성한 글은 젤 위(SNS) ? 젤 아래(카페/커뮤니티)
...
DELETE FROM BOARD
WHERE BOARD_NO = '1';
--무결성 제약조건(PJA_MEMBER.SYS_C008117)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
--> 기본적으로 삭제 불가 
--> 삭제 옵션을 추가 하면 가능 
--> ON DELETE SET NULL (부모삭제 시 자식 컬럼 NULL) ON DELETE CASCADE(부모삭제 시 참조하던 자식 행도 삭제) 

-- 제약 조건은 ALTER(변경) 없음 -> 삭제 후 다시 추가 

-- 기존 REPLY 테이블에 FK 제약 조건 삭제 (REPLY에 제약조건에서 확인)_
ALTER TABLE REPLY DROP CONSTRAINT SYS_C008117;
...
SELECT * FROM BOARD WHERE BOARD_NO = 1; -- 게시글 삭제 
SELECT * FROM REPLY WHERE BOARD_NO = 1; -- 해당 게시글 안에 있는 댓글도 삭제 
...
FROM BOARD B
JOIN MEMBER USING(MEMBER_NO)
WHERE BOARD_TITLE LIKE '%'||'민'||'%' --제목 검색

WHERE BOARD_CONTENT LIKE '%' || '민' || '%' --내용 검색

WHERE BOARD_TITLE LIKE '%'||'민'||'%' 
OR BOARD_CONTENT LIKE '%'||'민'||'%'  -- 제목 + 내용 검색
