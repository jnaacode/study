...
-- 테이블에 값을 삽입하거나(INSERT), 수정하거나(DUPATE), 삭제(DELETE)하는 구문
...
-- DEPARTMENT2 테이블에서 DEPT_ID가 'D9'부서 정보 조회 
...
-------------------------------------------------------------------------------
--* 조건절을 설정하지 않고 UPDATE구문 실행 시 모든 행의 컬럼 값 변경 
SELECT * FROM DEPARTMENT2;
...
-- * 여러 칼럼은 한번에 수정할 시 콤마(,)로 칼럼을 구분하면됨 
...
-- 만약 WHWERE 조건을 설정하지 않으면 모든 행이 다 삭제됨 
...
--EMPLOYEE2 테이블에서 이름이 '윤은해'이 사원 정보 삭제 
...
-- 5. TRUNCATE (DDL입니다! DML 아닙니다!)
-- 테이블의 전체 행을 삭제하는 DDL
-- DELETE보다 수행속도가 더 빠르다. 
-- ROLLBACK을 통해 복구 할 수 없다. (완전삭제개념)
