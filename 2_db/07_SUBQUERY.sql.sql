...
--2) 부서별로 급여 합이 17700000인 부서명과 급여 합 조회 
...
--3) 위에 두서브쿼리를 합 부서별 급여 합이 큰 부서의 부서명, 급여 합 조회 
SELECT DEPT_TITLE, SUM(SALARY)
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
GROUP BY DEPT_TITLE
...
-- IN을 사용하는이유는 = 했을때 1개의 값만 나와야하는데 사수 7명이 있기 때문에 =으로 작성하면 오류가 난다. 
...
--2) 직급이 과장인 직원들의 사번 이름 직급명 급여 조회 
SELECT EMP_ID,EMP_NAME, JOB_NAME, SALARY
FROM EMPLOYEE 
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME ='과장';
...
--1)LOCATION 테이블을 통해 NATIONAL_CODE가 KO인 LOCAL_CODE가 조회 
SELECT LOCAL_CODE
FROM LOCATION
...
--1. 노옹철 사원과 같은 부서, 같은 직급인 사원을 조회하세요 (단, 노옹철 사원은 제외)
--   사번, 이름, 부서코드,직급코드, 부서명, 직급명 

--SELECT  EMP_ID,EMP_NAME,DEPT_CODE,JOB_CODE,DEPT_TITLE,JOB_CODE
--FROM EMPLOYEE
--JOIN DEPARTMENT ON (DEPT_CODE =DEPT_ID)
--WHERE (EMP_NAME = '노옹철') (SELECT DEPT_CODE,JOB_CODE
--                                FROM EMPLOYEE);
...
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE =DEPT_ID)
NATURAL JOIN JOB --자연조인(칼럼명, 데이터 타입 일치하는 두 컬럼을 연결)
...
--                                WHERE SUBSTR(EMP_NO,8,1)=2
--                                AND EXTRACT(YEAR FROM EMP_NO)= 1977);
 
SELECT EMP_ID,EMP_NAME,DEPT_CODE,MANAGER_ID,EMP_NO,HIRE_DATE
FROM EMPLOYEE 
WHERE (DEPT_CODE,MANAGER_ID) = (SELECT DEPT_CODE,MANAGER_ID FROM EMPLOYEE
                                --WHERE SUBSTR(EMP_NO,1,8) LIKE '77_____2%');
...
-- 본인 직급의 평균 급여를 받고 있느 직원의 
-- 사번, 이름, 직급, 급여를 조회하시오
-- 단, 급여와 급여평균은 만원 단위로 계산하세요 TRUNC(칼럼명,-1)

--1) 급여를 200, 600만 받는 직원 (200만, 600만이 평균 급여라고 생각할 경우)
...
--서브쿼리
--SELECT AVG(SALARY) FROM EMPLOYEE E2 WHERE 'J2'=E2.JOB_CODE;
...
-- EXISTS : 서브쿼리에 해당하는 행이 1개라도 존재하면 조회 결과에 포함  -> 존재할 때 출력해라! -> 이태림 EMP_ID 100번,
...
--6.스칼라 서브쿼리 = 상관쿼리랑 같이 사용한다. 
--        SELECT절에 사용되는 서브쿼리 결과롤 1행(단일행)만 반환 
--        -> SELECT 절에 작성하는 단일행 서브쿼리 
--           SQL에서 단일 값을 가르켜 '스칼라'라고 함 

-- 현재 전체 평균을 얻어오고 있는 상태임 
...
FROM EMPLOYEE
ORDER BY SALARY DESC;
--> 해결 1의 조회결과(RESULT SET)을 가상의 테이블(==VIEW)로 취급 할 예정 
...
--급여 평균이 3위 안에 
-- 드는 부서의 부서코드와 부서명, 평균급여를 조회 
SELECT ROWNUM, DEPT_CODE,DEPT_TITLE,평균급여
                                        --평균급여 FLOOR(AVG(SALARY
...
--SELECT ROWNUM, DEPT_CODE,DEPT_TITLE,SALARY
--FROM (SELECT DEPT_CODE,DEPT_TITLE,SALARY
--            FROM EMPLOYEE
--            JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
--            ORDER BY SALARY DESC)
--WHERE ROWNUM <=3;
...
-- 8.WITH
-- 서브쿼리에 이름을 붙여주고 사용시 이름을 사용함
-- 인라인뷰로 사용될 서브쿼리에 주로 사용함
-- 실행 속도도 빨라진다는 장점이 있다. 
...
--TOP_SAL라는 이름의 서브쿼리를 미리 생성 (주문하자마자 만드는것이 아니라 미리 만들고 꺼내는 형태/ 블고기버거)
