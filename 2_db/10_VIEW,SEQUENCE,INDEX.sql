/*VIEW 
    ...
    - 논리적 가상 테이블
        -> 테이블 모양을 하고는 있지만, 실제값을 저장하고 있진 않음 
...
-- 각자 이니셜계정에 VIEW 생성권한을 부여 
-- (실행 시 관리자 계정으로 할 것) 

GRANT CREATE VIEW TO PJA; --Grant을(를) 성공했습니다.
...
--> 상기 오류가 발생한다면 OR REPLACE 옵션을 사용하면 된다. 

-- 별칭이 적용된 VIEW에서 직급이 '대리'인 직원만 조회 
--> VIEW에서 조회된 컬럼의 이름을 조건절에 사용해야한다 (JOB_NAME이라고 작성하면 오류난다!)
SELECT * FROM V_EMP
WHERE 직급명 ='대리';
...
--> 가상의 테이블인 VIEW에 테이블 삽입이 가능한걸까 ? -> D0이 삽입 된 것처럼 보이지만 실제로는 NO!!
...
-- VIEW를 이용한 DML 사용시 발생하는 문제점 == 제약조건 위배현상 
ROLLBACK;

SELECT * FROM DEPT_COPY2;
SELECT * FROM V_DCOPY2;

-- 원본 테이블 DEPT_TITLE칼럼에 NOT NULL 조약조건 추가 
ALTER TABLE DEPT_COPY2 MODIFY DEPT_TITLE NOT NULL; --NOT NULL일때 MODIFY라고!

-- 현 상태에서 다시 VIEW를 이용한 INSERT TNGOD 
INSERT INTO V_DCOPY2 VALUES ('D0','L3');
--ORA-01400: NULL을 ("PJA"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다.

-- VIEW를 이용한 INSERT 시 원본 테이블에 삽입이 된다.
-- 원본 테이블 삽입 시 VIEW INSERT 구문이 미포함된 컬럼에는 NULL이 저장된다. 
-- 그런데 DEPT_TITLE 칼럼에 NOT NULL 제약조건이 설정되어 있음 
--> 오류발생 ! VIEW를 이용한 INSERT 실패
...
CREATE OR REPLACE VIEW V_DCOPY2
AS SELECT DEPT_ID,LOCATION_ID FROM DEPT_COPY2
WITH READ ONLY; --읽기 전용의 VIEW 생성(DML 안됨)
...
/* SEQUENCE(순서, 연속)
  ...
  ***SEQUENCE 사용하는 방법 : PRIMARY KEY 칼럼에 사용될 값을 생성하는 용도로 사용 ***
...
--> 시퀀스 호출 시 미리 저장되어 진 값들을 가져와 반환하므로 매번 시퀀스를 생성해서 반환하는 것보다 DB속도가 향상된다.
...
--EMPLOYEE_COPY4의 EMP_ID 칼럼에 PK 제약조건 추가  --NOT NULL만 제약조건 추가 됨 나머지는 제약조건 추가해야함 
...
-- ORA-08002: 시퀀스 SEQ_EMP_ID.CURRVAL은 이 세션에서는 정의 되어 있지 않습니다 ( 스퀀스 생성 후 NEXTVAL 호출 없이 CURRVAL를 호출하면 오류 발생 )
 ...
 * 인덱스 장점 
 - 이진트리 형식으로 구서오디어 있어 자동 정렬 및 검색 속도가 빠름 
 - 조회 시 전체 테이블 내용을 조회하는 것이 아닌 인덱스가 지정된 컬럼만을 이용해서 조회하기 때문에 
   시스템 부하가 낮아져 전체적인 성능이 향상된다. 
   

 * 인덱스 단점 
 - 데이터 변경 (UPDATE,INSERT, DELETE) 작업이 빈번한 경우 오힐 성능이 저하되는 문제가 발생 
 - 인덱스도 하나이 객체이다 보니 이를 저장하기 위한 별도 공간 필요
 - 인덱스 생성 시간이 필요
...
CREATE TABLE TB_IDX_TEST(
    TEST_NO NUMBER PRIMARY KEY, --자동으로 인덱스 생성된다. 
...
WHERE TEST_ID = 'TEST500000'; --0.022초 

--인덱스 사용 
SELECT * FROM TB_IDX_TEST 
WHERE TEST_NO =50000; -- 0.002초 
