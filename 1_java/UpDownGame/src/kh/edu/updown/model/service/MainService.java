...
public class MainService {
	 private Scanner sc = new Scanner(System.in);
	   ...
	   public void signUp() { 
	  	...
	  	//memberCount가 members 배열 길이 이상인 경우 == 회원가입 정원이 다 찬 상태 
			if(memberCount >= members.length) {
				System.out.println("회원 정원이 가득 차 더 이상 가입을 할 수 없습니다.");
			}else {//회원 가입이 가능한 경우 
				...
				while(true) {
					...
					for(int i =0 ;i <members.length;i++) {
						if(members[i] != null) { //members 배열 요소가 null이 아닌 경우 == 회원정보가 있는 경우 
							
							//입력 받는 아이디와 같은 아이디가 members에 존재하는 경우 
							if(memberId.equals(members[i].getMemberId())) {
								System.out.println("중복되는 아이디 입니다. 다시 입력 해주세요.");
								
								flag = false; //바로 밑에 else 작성했어도 중복된 아이디가 없을 경우 종료되기에 flag를 사용한다. 
								break; //중복 검사 for문 종료 
							}
							
						}else {//members배열 요소가 null 인 경우 == 회원정보가 없는 경우 
							break; //중복 검사 for문 종료 
						}
					}
				...
				//입력 받을 값을 이용해서 새로운 Member객체를 생성하고 members 배열 중 
				//memberCount번째 요소가 참조 할수 있도록 주소를 저장한다. 
				
				members[memberCount] = new Member(memberId,memberPw,memberName);
				
				//새로운 회원이 가입되었으므로 memberCount를 1증가
				memberCount++;
	   ...
	   public void login() {
		...
		for(int i =0;i<members.length;i++) {
			if(members[i] !=null) { //members배열 요소가 null이 아닌경우 == 회원정보가 있는 경우 
				
				//members배열 요소 중 입력 받은 아이디, 비밀번호가 일치하는 회원이 있을 경우 
			...
			}else { //members 배열 요소가 null인 경우 == 회원정보가 없는 경우 
				break;
			}
		...
		//아이디, 비밀번호를 비교한 후에도 loginMember 필드가 null 인경우 == 로그인 실패
		//위에서 member[i]에 있는 정보를 loginMember에 담았는데 null이었다면 일치하는 경우가 없다! 
		if(loginMember == null) {
